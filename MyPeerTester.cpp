...
void map_tests()
{
	...
	breakFunc();
    std::cout << "size before insert : " << map.size() << std::endl;
    ft::map<int, char>::iterator it_007 = map.begin();
    for(;it_007 != map.end();it_007++)
    {
        std::cerr << it_007->first << std::endl;
    }
    ft::map<int, char>::iterator it_008 = map.begin();
    for(;it_008 != map.end();it_008++)
    {
        std::cerr << it_008->first << std::endl;
    }
    puts("kk");
    ft::map<int, char>::iterator it_22 = map.begin();
    while(it_22 != map.end())
    {
        puts("hatik m0wd");
        it_22++;
